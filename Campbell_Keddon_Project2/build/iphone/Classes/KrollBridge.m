...
NSString * Campbell_Keddon_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Campbell_Keddon_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_campbell_keddon_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Campbell_Keddon_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Campbell_Keddon_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_campbell_keddon_project2 = [[Campbell_Keddon_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_campbell_keddon_project2];
	
	NSString *_campbell_keddon_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _campbell_keddon_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_campbell_keddon_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Campbell_Keddon_Project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.campbell_keddon_project2.kroll" 
